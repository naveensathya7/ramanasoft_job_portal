...
const HrViewJobs = () => {
  ...
  const fetchJobs = async () => {
    try {
      ...
      axios.get("http://localhost:5000/view-jobs")
      .then(response=>{
  ...
  const handleSave = async(updatedJob) => {
    console.log("ud:",updatedJob)

    const changedValues={}
    console.log(jobs)
                const cand=filteredJobs.find(job=>updatedJob.jobId===job.jobId)
  ...
  return (
    <div style={{overflow:'hidden'}}>
          ...
          <Col key={job.jobId}>
            <div className="card h-100" style={{'box-shadow': '0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19)'}}>
              <div className="card-body">
                ...
                <p className='card-text'><span className='span-text'>LastDate:</span> {formatDate(job.lastDate)}</p>
      ...
      </Row>):(<h2 className='text-center text-secondary'>No jobs to display</h2>)}
    </Container>
    </div>
  );
};
